...
var vbluat = {
    ...
    events: function () {
            ...
            function(event) {
                ...
                $('#docPrint').lawsExists(function() {
                    $(this).printThis({
                        ...
                        loadCSS: vbluat.virtualPath('/assets/print.css')
                    });
                });
            });
        ...
        $('.wg-tieudiem').lawsExists(function () {
            var self = this,el = $('.singer.chitiet'), height = el.height(), top = el.offset().top;
            $(window).scroll(function () {
                var scroll = $(window).scrollTop();
                if (scroll > top && scroll < height) {
                    self.addClass('td-fixed');
                } else if (scroll <= top || scroll >= height) {
                    self.removeClass('td-fixed');
                }
            });
        });
                    ...
                    $(this).printThis({
                        ...
                        header:
                            '<div class="hearder-logo-print"><div class="logo-print"><img alt="van ban luat" src="' +
                                vbluat.virtualPath('/assets/images/logo.png') +
                                '"></div></div>'
    ...
    myDocuments: {
        ...
        $.icget({
            ...
            success: function (resp) {
                if (resp.Message != null && resp.Message.length > 0) {
                    $().icpopup({ messages: [resp.Message] });
                    vbluat.myDocuments.dllMyDocumentChange();
                }
            }
        });
        },
    ...
    gopy: function(docName) {
        $('<div id="gopy"></div>').lawsDialog({
            ...
            onOpen: function() {
                $('#gopy').load(vbluat.virtualPath('/Ajax/PartialGopY'),
                    function() {
        ...
        for (var i = 0; i < globalList.length; i++) {
            ...
            for (var j = 0; j < diList.length; j++) {
                ...
                if ((di.bottom >= pageTop) &&
                    ...
                    (di.top >= pageTop)) {
                    var position = $('#mucluc li.list-mluc a.' + di.obj[0].id).position().top;
                    setTimeout(function () {
                        $('.content-scroll-1').mCustomScrollbar('scrollTo', position);
                    }, 500);
                    $('#mucluc li.list-mluc a.' + di.obj[0].id).css({ 'color': '#0077bf !important','font-weight': 'bold'});
                } else if (pageTop <= di.top){
                    $('#mucluc li.list-mluc a.' + di.obj[0].id).removeAttr('style');
                }
            }
        }
    },
...
(function () {
    $.fn.posFixed = function (top,height) {
        var self = this;
        $(window).scroll(function () {
            var scroll = $(window).scrollTop();
            if (scroll > top && scroll < height) {
    ...
    $.fn.googleLogin = function(options) {
        var defaults = {
            ...
            redirectUrl: 'http://vbl.icsoft.vn',
                    ...
                    $.icpost({
                        ...
                        success: function (response) {
                            if (!response.Completed) {
                                $().icpopup({ messages: [response.Data.ActionMessage] });
                            }
    ...
    $.fn.facebookLogin = function (options) {
        var defaults = {
            ...
            showLogs: false,
            permissions: 'user_about_me,email',
            connected: false
        }
        options = $.extend(defaults, options);
        $(this).on('click',
    ...
    $.fn.vbluatInfiteScroll = function(options) {
        ...
        var hiddenTitleAndUrl = function (_title, _path, _page, _prev, _next) {
                return "<span class='hidden-title' style='display:none'>" +
                    ...
                    _page +
                    "</span>" +
                    "</span><span class='hidden-prev' style='display:none'>" +
                    _prev +
                    "</span>" +
                    "</span><span class='hidden-next' style='display:none'>" +
                    _next +
                    "</span>";
            },
            setTitleAndHistory = function (_title, _path, _prev, _next) {
                // Set history
                ...
                $('link[rel="alternate"]').attr('href', _path);
                seoPagination(_prev, _next);
            },
            changeTitleAndUrl = function (_value) {
                var _title = $(_value).children(".hidden-title:first").text(),
                    _path = $(_value).children(".hidden-url:first").text(),
                    _prev = $(_value).children(".hidden-prev:first").text(),
                    _next = $(_value).children(".hidden-next:first").text(); 
                    if ($('title').text() !== _title) {
                    // title co thay doi
                    setTitleAndHistory(_title, _path, _prev, _next);
                }
            },
            urlChangePage = function (_url, _page) {
                var regex = new RegExp("(page=)[0-9]+", 'ig');
                return _url.replace(regex, '$1' + _page);
            },
            seoPagination = function (_prev, _next) {
        ...
        var originalTitle = title = $('title').text(),
            ...
            originalUrl = path.indexOf('?') === -1 ? path : path.substr(0, path.indexOf('?')), page = $(settings.nextSelector).data('page'), prev = $(settings.nextSelector).data('prev'), next = $(settings.nextSelector).data('next'),
            documentHeight = $(document).height(),
            windowHeight = (typeof window.outerHeight !== 'undefined') ? Math.max(window.outerHeight, $(window).height()) : $(window).height(),
            $contents = $(settings.contentSelector);
        $(settings.contentSelector + ':last').append(hiddenTitleAndUrl(title, path, page, prev, next));
            ...
            function(e) {
                ...
                if (query[0] !== undefined && ajaxUrl[0] !== undefined) {
                    ...
                    $.icget({
                        ...
                        dataType: 'html',
                        success: function (resp) {
                            title = originalTitle + ' - trang ' + page; 
                            ...
                            $(settings.contentsWrapperSelector)
                                    .append($(resp).find(settings.contentSelector)
                                        .append(hiddenTitleAndUrl(title, originalUrl + path, el.data('page'), el.data('prev'), el.data('next'))))
                                    .append($(resp).find(settings.nextSelector).closest('.loadmore'));
                            documentHeight = $(document).height();
                            $contents = $(settings.contentSelector);
                            page = $(settings.nextSelector).data('page');
                            //prev = $(settings.nextSelector).data('prev');
                            //next = $(settings.nextSelector).data('next'); 
                        }
                    });
                }
            });
    ...
    $.fn.vbluatDetailFull = function(options) {
        ...
        var setTitleAndHistory = function (_title, _path) {
            ...
            return '';
                ...
                var height = $(opts.noidungTab).height(),
                top = $(opts.noidungTab).offset().top;
                $(opts.mucluc).posFixed(top,height);
            },
            ...
            if (activeTab != opts.luocdoTab && $(opts.mucluc).length) {
                $(window).scroll(docIndexScroll);
