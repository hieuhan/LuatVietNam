<%@ Page Title="" Language="C#" MasterPageFile="~/Admin/MasterPageAdmin.master" AutoEventWireup="true" CodeFile="DocsReviewStandard.aspx.cs" Inherits="Admin_DocsReviewStandard" %>
...
<asp:Panel ID="panSearch" runat="server" DefaultButton="btnSearch" Width="100%" >
    <table cellpadding="3" cellspacing="0" style="width:100%">        
        <tr>
            ...
            <asp:Label ID="lblDataSources" runat="server" meta:resourcekey="lblDataSources" Visible="false"
                ...
                <asp:Label ID="Label1" runat="server" Text="Tỉnh thành:" Visible="false"></asp:Label>
            ...
            <td style="width:250px;">
                ...
                <asp:DropDownList ID="ddlProvinces" Visible="false" runat="server" AutoPostBack="False" CssClass="userselect uiselect"
	...
	<div class="chucnangright">
         <a href="VietNamStandardsEdit.aspx?LanguageId=<%=LanguageId %>&backUrl=DocsReviewStandard.aspx" title="Thêm mới tiêu chuẩn Việt Nam" class="themmoi" > 
        ...
        <asp:LinkButton ID="lbDelete" runat="server" CssClass="xoatin" Text="Xóa"  OnClientClick="return confirm('Bạn có chắc chắn muốn xóa các văn bản đã chọn?')"  
            meta:resourcekey="lbDelete" ToolTip="Xóa các tiêu chuẩn Việt Nam được tick chọn và chưa duyệt" onclick="lbDelete_Click">
        </asp:LinkButton>
	</div>
        ...
        <asp:GridView ID="m_grid" DataKeyNames="DocId" runat="server" ShowHeaderWhenEmpty="True"
            ...
            <Columns>                        
                ...
                <asp:TemplateField > 
                    ...
                    <ItemTemplate> 
                       <a  title="Sửa tiêu chuẩn này" runat="server"  href='<%# "VietNamStandardsEdit.aspx?DocId=" + Eval("DocId").ToString() + "&LanguageId=" + Eval("LanguageId").ToString()+ "&backUrl=DocsReviewStandard.aspx"  %>'
                               class="docsproperties" ><asp:Label ID="lbDocName" runat="server" Text='<%# Eval("DocName")%>' style="font-weight:bold; color:#036AA4;"></asp:Label> </a>
                        <br />
                        <br />
                          <a WHeight="480" WWidth="950" title="Sửa thuộc tính" runat="server"  href='<%# "DocsStandardEditAttribute.aspx?DocId=" + Eval("DocId").ToString() + "&LanguageId=" + Eval("LanguageId")  %>'
                               class="docsproperties popup" onmouseover='<%# "ShowTooltip(" + chr + "docstips.aspx?DocId=" + Eval( "DocId") +"&LanguageId=" + Eval( "LanguageId") + chr +"," + chr + "tt" + chr + "); return false;" %>'
                               onmouseout='HideTooltip();' EnableViewState="False"  >
                              <asp:Label ID="lblGridColumnProperties" runat="server" CssClass="docsproperties" Text="Thuộc tính" 
                                  meta:resourcekey="lblGridColumnProperties" ></asp:Label></a>
                        <span class="docsproperties"> | </span>  

                        <a WHeight="600" WWidth="1050" href='<%# "DocRelatesVietNamStandard.aspx?DocId=" + Eval("DocId").ToString() + "&LanguageId=" + Eval("LanguageId") %>' class="docsproperties popup <%#HasProperties(Eval("HasDocRelate").ToString())  ? "":"active" %>" title="<%=GetLocalResourceObject("lblGridColumnDocRelateEdit.title").ToString() %>"><asp:Label ID="lblGridColumnLienQuan" runat="server" CssClass="docsproperties" 
                              Text="Liên quan" meta:resourcekey="lblGridColumnLienQuan"></asp:Label></a>
                        <span class="docsproperties"> | </span> 

                        <a  WHeight="500" WWidth="850" href='<%# "DocsFielsEdit.aspx?DocId=" + Eval("DocId").ToString() + "&LanguageId=" + Eval("LanguageId") %>' class="docsproperties popup <%#HasProperties(Eval("HasFile").ToString())  ? "":"active" %>"  title="<%=GetLocalResourceObject("lblGridColumnFile.title").ToString() %>"><asp:Label ID="lblGridColumnFile" runat="server" CssClass="docsproperties" 
                              Text="File" meta:resourcekey="lblGridColumnFile"></asp:Label></a> 
                        <span class="docsproperties"> | </span> 

                        <a  href='<%# "DocsUpdateContentEdit.aspx?DocId=" + Eval("DocId").ToString() + "&LanguageId=" + Eval("LanguageId")+ "&backUrl=DocsReviewStandard.aspx" %>' class="docsproperties <%#HasProperties(Eval("HasContent").ToString())  ? "":"active" %>" title="<%=GetLocalResourceObject("lblGridColumnContent.title").ToString() %>">
                            <asp:Label ID="lblGridColumnContent" runat="server" CssClass="docsproperties" 
                              Text="Xem sửa nội dung" meta:resourcekey="lblGridColumnContent"></asp:Label></a>
                        <span class="docsproperties"> | </span> 
                         
                        <a href='<%# "DocIndexes.aspx?DocId=" + Eval("DocId").ToString() + "&LanguageId=" + Eval("LanguageId").ToString()+ "&backUrl=DocsReviewStandard.aspx" %>' class="docsproperties <%#HasProperties(Eval("HasDocIndex").ToString())  ? "":"active" %>" title="Mục lục văn bản">
                            <asp:Label ID="Label1" runat="server" CssClass="docsproperties" Text="Mục lục" ></asp:Label>
                        </a><span class="docsproperties"> | </span>
                           
                        <a href='<%# "DocsContentRelation.aspx?DocId=" + Eval("DocId").ToString() + "&LanguageId=" + Eval("LanguageId").ToString()+ "&backUrl=DocsReviewStandard.aspx" %>' class="docsproperties <%#HasProperties(Eval("HasDocRelate").ToString())  ? "":"active" %>"  title="<%=GetLocalResourceObject("lblGridColumnContentRelation.title").ToString() %>"><asp:Label ID="lblGridColumnContentRelation" runat="server" CssClass="docsproperties" 
                              Text="Lược đồ" meta:resourcekey="lblGridColumnContentRelation"></asp:Label></a>   <span class="docsproperties"></span> <span class="docsproperties"> | </span> 
                        ...
                        <table border="0px" cellpadding="0" cellspacing="0">
                            <tr>
                                <td style="width:60px; text-align:left;"><asp:Label ID="lblGridColumnBH" CssClass="properties" runat="server" Text="BH:" meta:resourcekey="lblGridColumnBH"></asp:Label></td>
                                 <td><span style="color:Black;"> <%# String.IsNullOrEmpty(DateTimeHelpers.GetDateHH24(Eval("IssueDate")))?Eval("Fee"):DateTimeHelpers.GetDateHH24(Eval("IssueDate"))%></span></td>
                            </tr>
                            <tr>
                ...
                <asp:TemplateField  > 
                    ...
                    <ItemTemplate> 
                       <span class="xuatban<%# Eval("ReviewStatusId") %>" > <%# LanguageHelpers.IsCultureVietnamese() ? ReviewStatus.Static_Get(byte.Parse(Eval("ReviewStatusId").ToString()), l_ReviewStatus).ReviewStatusDesc : ReviewStatus.Static_Get(byte.Parse(Eval("ReviewStatusId").ToString()), l_ReviewStatus).ReviewStatusName%></span><br />
                         <a WHeight="400" WWidth="550" href="<%=CmsConstants.ROOT_PATH %>admin/pages/WebNotify/NotifyMessagesEdit.aspx?DocId=<%# Eval("DocId") %>&LanguageId=<%# Eval("LanguageId") %>&BackUrl=Pages/lawsdocs/DocsReviewStandard.aspx" class="popup" style="color:blue;" title="Gửi Web Notify"> Gửi Notify</a> 
                    </ItemTemplate>
                ...
                <asp:TemplateField>
                    ...
                    <HeaderStyle Width="3%" />
                    <HeaderTemplate>
                        <input type="checkbox" name="SelectAllCheckBox" onclick="SelectAll(this)" title="Chọn tất cả">
                    </HeaderTemplate>
                    <ItemTemplate>
