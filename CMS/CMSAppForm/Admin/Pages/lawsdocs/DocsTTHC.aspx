<%@ Page Title="" Language="C#" MasterPageFile="~/Admin/MasterPageAdmin.master" AutoEventWireup="true" CodeFile="DocsTTHC.aspx.cs" Inherits="Admin_DocsTTHC" %>
        ...
        <asp:GridView ID="m_grid" DataKeyNames="DocId" runat="server" ShowHeaderWhenEmpty="True"
            ...
            <Columns>                        
                ...
                <asp:TemplateField > 
                    ...
                    <ItemTemplate> 
                        <a title="click để sửa thủ tục hành chính này" href="<%# "DocsEditTTHC.aspx?DocId=" + Eval("DocId").ToString() + "&LanguageId=" + Eval("LanguageId")  %>" >
                        <asp:Label ID="lbDocName" runat="server" Text='<%# Eval("DocName")%>' style="font-weight:bold;"></asp:Label>                        
                        </a>
                        <br />
                        <br />
                        <a title="Sửa thông tin thủ tục hành chính" WHeight="650" WWidth="1200" href='<%# "DocsEditInfo.aspx?DocId=" + Eval("DocId").ToString() + "&LanguageId=" + Eval("LanguageId")  %>' class="docsproperties popup">
                            Thông tin
                        </a>
                        ...
                        <a WHeight="400" WWidth="850" href='<%# "DocsFielsEdit.aspx?DocId=" + Eval("DocId").ToString() + "&LanguageId=" + Eval("LanguageId") %>' class="docsproperties popup <%#HasProperties(Eval("HasFile").ToString())  ? "":"active" %>"  title="<%=GetLocalResourceObject("lblGridColumnFile.title").ToString() %>">
               ...
               <asp:TemplateField  > 
                    ...
                    <ItemTemplate> 
                       <span class="xuatban<%# Eval("ReviewStatusId") %>" > <%# LanguageHelpers.IsCultureVietnamese() ? ReviewStatus.Static_Get(byte.Parse(Eval("ReviewStatusId").ToString()), l_ReviewStatus).ReviewStatusDesc : ReviewStatus.Static_Get(byte.Parse(Eval("ReviewStatusId").ToString()), l_ReviewStatus).ReviewStatusName%></span><br />
                         <a WHeight="400" WWidth="550" href="<%=CmsConstants.ROOT_PATH %>admin/pages/WebNotify/NotifyMessagesEdit.aspx?DocId=<%# Eval("DocId") %>&LanguageId=<%# Eval("LanguageId") %>&BackUrl=Pages/lawsdocs/DocsTTHC.aspx" class="popup" style="color:blue;" title="Gửi Web Notify"> Gửi Notify</a>    
                    </ItemTemplate>
                    <ItemStyle Width="5%" HorizontalAlign = "Left" VerticalAlign="Top" Wrap="false" />  
                    <HeaderStyle Width="5%" />        
                </asp:TemplateField>
