...
namespace Processor
{
    public partial class Processor : Form
    {
       ...
       private void ProcessFolder()
       {
           ...
           ListDocs = mDocs.GetDocContainsHeadMeta(100);
           if (ListDocs.Count > 0)
           {
               foreach (Docs m_Docs in ListDocs)
               {
                   UpdateDocContent(m_Docs);
               }
           }

       }
       private void UpdateDocContent(Docs m_Docs)
       {
           short message = 0;
           string path = txtRootFolder.Text;
           try
           {
               string content = m_Docs.DocContent;
               //content = Regex.Replace(content, "<!–(.|\\s)*?–>", "");
               // Remove HTML tags
                //content = Regex.Replace(content, "</?[a-z][a-z0-9]*[^<>]*>", "");
               ...
               _htmlDoc = GetHtmlDocument(content);
               

               string style = _htmlDoc.DocumentNode.SelectSingleNode("//head/style") == null ? "" : _htmlDoc.DocumentNode.SelectSingleNode("//head/style").InnerHtml;
               string body = _htmlDoc.DocumentNode.SelectSingleNode("//body") == null ? _htmlDoc.DocumentNode.InnerHtml : _htmlDoc.DocumentNode.SelectSingleNode("//body").InnerHtml;

               
               if(!string.IsNullOrEmpty(body))
               {
                   LogControl.WriteMessage(txtLog, "Execute DocsId: " + m_Docs.DocId);
                   content = "";
                   ...
                   content += body;
                   m_Docs.DocContent = content;
                   m_Docs.UpdateContent(0, 0, ref message);
               }
               else
               {
                   LogControl.WriteMessage(txtLog, "DocsId: " + m_Docs.DocId + " not found body meta!");
               }
       ...
       private HtmlAgilityPack.HtmlDocument GetHtmlDocument(string source)
       {
