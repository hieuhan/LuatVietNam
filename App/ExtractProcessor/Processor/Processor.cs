...
namespace Processor
{
    public partial class Processor : Form
    {
        ...
        public Processor()
        {
            ...
            txtOutputFolder.Text = ConfigurationManager.AppSettings["DATAFOLDER"];
        }
       ...
       private void ProcessFolder()
       {
           ...
           FileInfo[] l_FileInfo = new DirectoryInfo(txtRootFolder.Text).GetFiles();
           for (int index = 0; index < l_FileInfo.Length; index++)
           {
               //if (index >= 1)
               //    break;
                ProcessFileInput(l_FileInfo[index].FullName);
               //Thread.Sleep(SleepTime);
           }
        ...
        private void ProcessFile(string FilePath, DocsProcessAuto m_DocsProcessAuto)
       {
           ...
           try
           {
               ...
               {
                   OutputPath = "\\VIETLAWFILE\\" + ConstantProcessors.DefaultOutputFolder + "\\";

               }
               else
               {
                   OutputPath = "\\VIETLAWFILE\\" + m_WordDocuments.PublicDate.ToString("yyyy") + "\\" + m_WordDocuments.PublicDate.ToString("MM") + "\\";

               }
                OutputOriginalAbsoluteFile = "Uploaded" + OutputPath.Replace("\\","/") + m_FileInfo.Name;
                OutputHtmlAbsoluteFile = "Uploaded" + OutputPath.Replace("\\", "/") + CurentFileNameNoExt + ".html";
                OutputPdfAbsoluteFile = "Uploaded" + OutputPath.Replace("\\", "/") + CurentFileNameNoExt + ".pdf";
               ...
               if (Directory.Exists(OutputPath) == false)
               {
                ...
                File.Copy(m_FileInfo.FullName, OutputOriginalFile,true);
                // save
                ...
                if(m_Docs.DocId > 0)
                {
                    ...
                    m_DocFiles.FileSize = (int)(m_FileInfo.Length);
                    ...
                    if (!String.IsNullOrEmpty(htmlContent))
                    {
                        ...
                        foreach (string RelateNo in m_WordDocuments.l_DocNoRelated)
                        {
                            if (processNo.Contains(";" + RelateNo))
                                continue;
                            LogControl.WriteMessage(txtLog, RelateNo);
                            List<Docs> l_Docs = m_Docs.Docs_GetListByDocIdentity(1, 10, "", m_WordDocuments.LanguageId, RelateNo, 0);
                            if (l_Docs.Count == 1)
                            {
                                Docs m_DocsTemp = l_Docs[0];
                                string link = m_DocsTemp.DocUrl; //DocsView.GetDocUrl(m_DocsTemp.DocId, m_DocsTemp.DocName);
                                string ReferHtml = "<a class='docnorefer' href='" + link + "'>" + RelateNo + "</a>";
                                if(ConstantProcessors.DocNoLinkTarget != "")
                                    ReferHtml = "<a target='" + ConstantProcessors.DocNoLinkTarget + "' class='docnorefer' href='" + link + "'>" + RelateNo + "</a>";
                        ...
                        //end relate
                        htmlContent = htmlContent.Replace(CurentFileNameNoExt + "_files", ICSoft.CMSLib.CmsConstants.WEBSITE_IMAGEDOMAIN + OutputOriginalAbsoluteFile.Replace(m_FileInfo.Name,"") + CurentFileNameNoExt + "_files");
